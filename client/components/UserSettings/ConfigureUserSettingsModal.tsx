...
export const ConfigureUserSettingsModal: React.FC<ModalProps> = ({ show, onClose }) => {
    ...
    const [enableEmailNotifications, setEnableEmailNotifications] = useState(user.enableEmailNotifications);
    const [email, setEmail] = useState(user.email ?? "");
    const [interestedInSources, setInterestedInSources] = useState<InfoSourceType[]>(user.interestedInSources);
            ...
            <ModalContent>
                ...
                <ModalBody pb="1rem">
                    {user.state !== UserState.Trial &&
                        <>
                            <FormControl mt="1rem">
                                <Checkbox isChecked={enableEmailNotifications} onChange={e => setEnableEmailNotifications(e.target.checked)}>
                            ...
                            </FormControl>
                            {
                                ...
                                <FormControl variant="floating" mt="2rem" isInvalid={!email}>
                                    ...
                                    {!email && <FormErrorMessage>This field is required</FormErrorMessage>}
                                </FormControl>
                            }
                        </>
                    }
                    <Flex align={["start", "center"]} direction={["column", "row"]} mt={["1rem", "1rem", "2rem"]} mb="1rem">
                    ...
                    <Flex justify="flex-end" width="100%" mt="2rem">
                        ...
                        <Button
                            ...
                            type="submit"
                            isLoading={loading}
                            disabled={loading || (user.state !== UserState.Trial && enableEmailNotifications && !email)}
