...
const Score: React.FC<{ score: number, displayScore: number }> = ({ score, displayScore }) => {
    ...
    return (
        <StatNumber pl="1.2rem">
            <Text
                display="initial"
                fontSize={isNaN(displayScore) ? '0.9rem' : '1rem'}
                py={isNaN(displayScore) ? '0.6rem' : '0.4rem'}
                px="0.4rem"
                bg={scoreColor}
            >
        ...
        <InfoSourceWrapper>
            <Box flex="1">
                <Stat>
                    ...
                    <Score
                        score={parseInt(data.criticScore)}
                        displayScore={parseInt(data.criticScore)}
                    />
                </Stat>
            </Box>
            <Box flex="1">
                <Stat>
                    ...
                    <Score
                        score={parseFloat(data.userScore) * 10}
                        displayScore={parseFloat(data.userScore)}
                    />
                </Stat>
            </Box>
        </InfoSourceWrapper>
