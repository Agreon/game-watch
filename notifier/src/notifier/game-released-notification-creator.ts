...
export class GameReleasedNotificationCreator implements NotificationCreator<NotificationType.GameReleased> {
    public forNotificationType: NotificationType.GameReleased;
    public supportsInfoSourceTypes: InfoSourceType[] = StoreInfoSources;

    public async createNotification(
        { infoSource, existingGameData, resolvedGameData, em, game, logger }: NotificationCreatorContext<StoreInfoSource>
    ) {
        ...
        if (dayjs(game.createdAt).isAfter(resolvedGameData.releaseDate)) {
            logger.debug(
                { context: { gameCreatedAt: game.createdAt, releaseDate: resolvedGameData.releaseDate } },
        ...
        if (dayjs(infoSource.foundAt).isAfter(resolvedGameData.releaseDate)) {
            logger.debug(
                { context: { sourceFoundAt: infoSource.foundAt, releaseDate: resolvedGameData.releaseDate } },
        ...
        logger.debug(
            { context: { gameCreatedAt: game.createdAt, releaseDate: resolvedGameData.releaseDate, existingNotification } },
