...
export class NotificationService {
  ...
  public async createNotifications({
    ...
    const relevantNotificationCreators = this.notificationCreators.filter(
      ({ supportsInfoSourceTypes }) =>
        supportsInfoSourceTypes.includes(infoSource.type),
    );
    ...
    await Promise.all(
      relevantNotificationCreators.map(
        async (creator) =>
  ...
  private async persistNotification({
    ...
    await this.em.transactional(async (transactionEm) => {
      ...
      if (user.enableEmailNotifications && user.emailConfirmed) {
        logger.info(
          `Sending notification of type ${notification.type} to ${user.email}`,
        );
