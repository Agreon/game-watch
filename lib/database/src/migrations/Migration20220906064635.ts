...
export class Migration20220906064635 extends Migration {
  async up(): Promise<void> {
    this.addSql(`
      ...
      add column "state" text check("state" in ('Found', 'Resolved', 'Error', 'Disabled')) not null default 'Found'
    `);
    ...
    this.addSql(`
      update "info_source" set "continue_searching" = true
      where "disabled" = true and "remote_game_id" is not null
    `);
