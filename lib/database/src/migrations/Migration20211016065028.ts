...
export class Migration20211016065028 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('alter table "game_tags" add constraint "game_tags_game_id_foreign" foreign key ("game_id") references "game" ("id") on update cascade on delete cascade;');
    this.addSql('alter table "game_tags" add constraint "game_tags_tag_id_foreign" foreign key ("tag_id") references "tag" ("id") on update cascade on delete cascade;');
  }
