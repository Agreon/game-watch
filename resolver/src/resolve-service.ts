...
export class ResolveService {
  public constructor(
    ...
    private readonly createNotificationsQueue: Queue<
      QueueParams[QueueType.CreateNotifications]
    >,
  ...
  public async resolveSource({
    ...
    try {
      ...
      const thrownError: Error & { code?: string } =
      ...
      await this.setResolveError({ source, triggeredManually });
      throw new CriticalError(source.type, thrownError);
    }
  }
  ...
  private async resolveGameInformation(
    ...
    const resolvedData =
      await this.getResolverForTypeOrFail(type).resolve(context);
  ...
  private getResolverForTypeOrFail(type: InfoSourceType): InfoResolver {
    const resolverForType = this.resolvers.find(
